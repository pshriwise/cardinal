...
[Postprocessors]
  ...
  [nekRS_posGradPres] # required for overlap coupling in SAM
    ...
    scaling_factor = 2.0 # one divided by length of coupled section
    execute_on = 'NONLINEAR TIMESTEP_BEGIN TIMESTEP_END'
  [../]
  ...
  [nekRS_accelGradPres]
    ...
    scaling_factor = 498780.5 # rho/dt (used in SAM)
    execute_on = 'TIMESTEP_END'
  [../]
...
[Transfers]
  ...
  [nekRS_avgUvolOld]
    ...
    multi_app = nek
    reduction_type = average
    from_postprocessor = nekRS_avgUvolOld
    to_postprocessor   = nekRS_avgUvolOld
    execute_on = 'TIMESTEP_END'
  [../]
...
[Executioner]
  ...
  dtmin = 2e-3                        # The allowed minimum time step size
  num_steps = 50000
